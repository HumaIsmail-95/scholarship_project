...
class StudentRequest extends FormRequest
{
    ...
    public function rules(Request $request)
    {
        ...
        if ($request->travel_history == 0) {
            ...
            $rules['travel_document'] = ['required', 'max:2048', 'mimes:jpg,bmp,png,pdf'];
        }
        $rules =  [
            'title' => ['required'],
            'name' => ['required'],
            ...
            'mobile' => ['required', ValidationRule::unique('students')->ignore($route_id)],
            'alternative_modile' => ['required'],
            'gender' => ['required'],
            'd_o_b' => ['required'],
            'id_type' => ['required'],
            'id_number' => ['required'],
            'id_document' => ['required', 'image', 'max:2048', 'mimes:jpg,bmp,png,pdf'],
